...
def get_agnews_mixed_data():
    if not os.path.exists(AGNEWS_DATA_PATH):
        ...
        ft_split = [
            [int(traintext_perclass_mid[0] * 0.9), int(len(traintext_perclass[1]) * 0.9)],
            [int(len(traintext_perclass[0]) * 0.9), int(traintext_perclass_mid[1] * 0.9)],
            [int(traintext_perclass_mid[2] * 0.9), int(len(traintext_perclass[3]) * 0.9)],
            [int(len(traintext_perclass[2]) * 0.9), int(traintext_perclass_mid[3] * 0.9)],
        ]

        traindata = [
            traintext_perclass[0][:ft_split[0][0]] + traintext_perclass[1][traintext_perclass_mid[1]:ft_split[0][1]],
            traintext_perclass[0][traintext_perclass_mid[0]:ft_split[1][0]] + traintext_perclass[1][:ft_split[1][1]],
            traintext_perclass[2][:ft_split[2][0]] + traintext_perclass[3][traintext_perclass_mid[3]:ft_split[2][1]],
            traintext_perclass[2][traintext_perclass_mid[2]:ft_split[3][0]] + traintext_perclass[3][:ft_split[3][1]],
        ]
        testdata = [
        ...
        ftdata = [
            traintext_perclass[0][ft_split[0][0]:traintext_perclass_mid[0]] + traintext_perclass[1][ft_split[0][1]:],
            traintext_perclass[0][ft_split[1][0]:] + traintext_perclass[1][ft_split[1][1]:traintext_perclass_mid[1]],
            traintext_perclass[2][ft_split[2][0]:traintext_perclass_mid[2]] + traintext_perclass[3][ft_split[2][1]:],
            traintext_perclass[2][ft_split[3][0]:] + traintext_perclass[3][ft_split[3][1]:traintext_perclass_mid[3]],
        ]

        for i in range(4):
            traintext = ' '.join(traindata[i])
            testtext = ' '.join(testdata[i])
            fttext = ' '.join(ftdata[i])
            raw_tokenized_train = tokenizer.encode_ordinary(traintext)
            raw_tokenized_eval = tokenizer.encode_ordinary(testtext)
            raw_tokenized_ft = tokenizer.encode_ordinary(fttext)
