...
def get_three_multi_data_full_star():
    ...
    if not os.path.exists(MULTI_DATA_PATH):
        ...
        for i in range(num_clients):
            ...
            end_train = ((i // 3) + 1) * 1500
            traindata.append(traintext_perclass[i % 3][start:end_train])
            start = (i // 3) * 300 # 160000 tokens
            end_test = ((i // 3) + 1) * 300
            testdata.append(testtext_perclass[i % 3][start:end_test])
        ...
        for i in range(num_clients, num_clients + 3):
            ...
            reftext = ' '.join(testtext_perclass[i % 3][end_test:end_test + diff])
            end_test += diff
            raw_tokenized_ref = tokenizer.encode_ordinary(reftext)
            ref_data.append(np.array(raw_tokenized_ref, dtype=np.uint16)[:300000])

            diff = (i // 3) * 2000  # 1600000 tokens
            star_text = ' '.join(traintext_perclass[i % 3][end_train:end_train + diff])
            raw_tokenized_star = tokenizer.encode_ordinary(star_text)
            train_star_data.append(np.array(raw_tokenized_star, dtype=np.uint16)[:280000])

            diff = (i // 3) * 2000  # 1600000 tokens
            star_text = ' '.join(testtext_perclass[i % 3][end_test:end_test + diff])
            raw_tokenized_star = tokenizer.encode_ordinary(star_text)
            test_star_data.append(np.array(raw_tokenized_star, dtype=np.uint16)[:53333])
        ...
        star_train_tokenized = np.concatenate(train_star_data)
        star_test_tokenized = np.concatenate(test_star_data)

        print(f'{star_train_tokenized.shape} train star, {star_test_tokenized.shape} test star')

        star_train_tokenized.tofile(os.path.join(MULTI_DATA_PATH, f'star_train.bin'))
        star_test_tokenized.tofile(os.path.join(MULTI_DATA_PATH, f'star_test.bin'))
    ...
    train_data.append(np.memmap(os.path.join(MULTI_DATA_PATH, f'star_train.bin'), dtype=np.uint16, mode='r'))
    val_data.append(np.memmap(os.path.join(MULTI_DATA_PATH, f'star_test.bin'), dtype=np.uint16, mode='r'))
